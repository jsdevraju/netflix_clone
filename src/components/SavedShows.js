...
const SavedShows = () => {
  ...
  useEffect(() => {
    onSnapshot(doc(db, "users", `${user?.email}`), (myDoc) => {
      setMovies(myDoc.data()?.saveShows);
    });
  }, [user?.email]);
        ...
        >
          {movies &&
            movies.map((item, id) => (
              <div
                key={id}
                className="w-[160px] sm:w-[220px] lg:w-[280px] inline-block cursor-pointer relative p-2"
              >
                <img
